...
c ... Demand that net feex cannot be out of the plates
     ...
     .                (1.+ (feix(ixrb(jx),iy)/feixflr)**4)
      ...
      if (capfeex.gt.0) then
      do  iy = j2, j5
      do  ix = i2, i5
      ix1 = ixm1(ix,iy)
      if (feex(ix,iy).gt.0) then
      feex(ix,iy)=feex(ix,iy)*pwrecapfac(ix,iy)
      endif
      if (feex(ix1,iy).lt.0) then
      feex(ix1,iy)=feex(ix1,iy)*pwrecapfac(ix,iy)
      endif
      if (feey(ix,iy).gt.0) then
      feey(ix,iy)=feey(ix,iy)*pwrecapfac(ix,iy)
      endif
      if (feex(ix,iy-1).lt.0) then
      feey(ix,iy-1)=feey(ix,iy-1)*pwrecapfac(ix,iy)
      endif
      ...
      if (capfeix.gt.0) then
      do  iy = j2, j5
      do  ix = i2, i5
      ix1 = ixm1(ix,iy)
      if (feix(ix,iy).gt.0) then
      feix(ix,iy)=feix(ix,iy)*pwricapfac(ix,iy)
      endif
      if (feix(ix1,iy).lt.0) then
      feix(ix1,iy)=feix(ix1,iy)*pwricapfac(ix,iy)
      endif
      if (feiy(ix,iy).gt.0) then
      feiy(ix,iy)=feiy(ix,iy)*pwricapfac(ix,iy)
      endif
      if (feix(ix,iy-1).lt.0) then
      feiy(ix,iy-1)=feiy(ix,iy-1)*pwricapfac(ix,iy)
      endif
 ...
 316        continue
      ...
      do iy = iys1, iyf6  #j2, j5
        do ix = ixs1, ixf6  #i2, i5
          vsoreec(ix,iy) =
     .          - cfneut*cfneutsor_ee*cnsor*13.6*ev*fac2sp*psorc(ix,iy,1)*pwrecapfac(ix,iy)
     .          + cfneut*cfneutsor_ee*cnsor*13.6*ev*fac2sp*psorrgc(ix,iy,1)
     .          - cfneut*cfneutsor_ee*cnsor*erliz(ix,iy)*pwrecapfac(ix,iy)
     .          - cfneut*cfneutsor_ee*cnsor*erlrc(ix,iy)*pwrecapfac(ix,iy)
     .          - cfneut*cfneutsor_ee*cnsor*ediss*ev*(0.5*psordis(ix,iy))*pwrecapfac(ix,iy)
        enddo
      enddo
     ...
     .                             vsoreec(ix2,iy)/vol(ix2,iy) )
      ...
      do 152 iy = j2, j5
         do 151 ix = i2, i5
            ...
            if (w0(ix,iy).gt.0) then
             w0(ix,iy)=w0(ix,iy)*pwrecapfac(ix,iy)
            else
             w0(ix,iy)=w0(ix,iy)*pwricapfac(ix,iy)
            endif
...
c ... If molecules are present as gas species 2, add ion/atom cooling
      if(ishymol == 1) then
        do iy = j2, j5
          do ix = i2, i5
          if (ti(ix,iy).gt.tg(ix,iy,2)) then
           capfac=pwricapfac(ix,iy)
          else
          capfac=1
          endif
            resei(ix,iy) = resei(ix,iy) - vol(ix,iy)*eqpg(ix,iy,2)*
     .                                     (ti(ix,iy)-tg(ix,iy,2))*capfac
          enddo
        enddo
      endif
...
*  -- Energy transfer to impurity neutrals at tg(,,igsp)
     ...
     .                      (ti(ix,iy) - tg(ix,iy,2))*vol(ix,iy)*capfac
            enddo
          enddo
        enddo
      endif
...
*  -- impurity radiation --
     ...
     .                     emissbs(te(ix,iy), nratio(ix,iy), ntau(ix,iy))
        ...
        pwrzec(ix,iy)=pwrzec(ix,iy)*pwrecapfac(ix,iy)
 532        continue
 533     continue
...
*  -- joule heating --
     ...
     .                                (phi(ix,iy)+phi(ix,iy-1))
               resee(ix,iy) = resee(ix,iy) + wjdote(ix,iy)*pwrecapfac(ix,iy) / ( 1. +
     ...
     .                             ey(ix,iy-1)*fqy(ix,iy-1) )/gy(ix,iy)
               resee(ix,iy) = resee(ix,iy) + wjdote(ix,iy)*pwrecapfac(ix,iy)
             enddo
           enddo
         endif
      endif
