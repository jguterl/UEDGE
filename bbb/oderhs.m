...
c ... Work-array argument:
      ...
      real,allocatable,save::wk_(:),igyl_(:,:),yl_(:),yldot00_(:)
c ... Output arguments:
      real jac(nnzmx)     # nonzero Jacobian elements
      integer ja(nnzmx)   # col indices of nonzero Jacobian elements
      integer ia(neq+1)   # pointers to beginning of each row in jac,ja
...
c      nii=max(max(2*4*numvar*nx+1,2*2*numvar*(nx+3)+1),mu+ml)
c      nii=lenpfac+2
      allocate(iJacCol(nnzmx,Nthreads))
      allocate(iJacRow(neq,Nthreads))
      allocate(rJacElem(nnzmx,Nthreads))
      allocate(nnzfinal(Nthreads))
      allocate(nnzcum(Nthreads))
cJG      igyl_(1:neq,1:2)=igyl(1:neq,1:2)
cJG       wk_(1:neq)=wk(1:neq)
cJG       yl_(1:neq)=yl(1:neq)
      write(*,*) "*** Init calc jacobian with neq=",neq
c # init. Not really needed...
c      do iv=1,neq
c       do jv=1,Nthreads
c        iJacConstructor(iv,jv)=-1
c        rJacConstructor(iv,jv)=0.0
c       enddo
c      enddo
      write(*,*) "*** End init calc jacobian"
...
cc$OMP target
cc$OMP& map(to:iv,time0,TID,ii1,ii2,yldot00,wk,igyl,iv,ii,nnz)
cc$OMP& map(to:yl(1:neq),yold,dyl,del,dylconst,suscal)
cc$OMP& map(to:dtuse,iseqalg,jacelem,idxphi,dtphi,sfscal)
cc$OMP& map(to:jaccliplim,nnzmx)
cc$OMP& map(from:iJacConstructor,rJacConstructor)
cc$omp teams num_teams(Nthreads)

ccc$OMP parallel do schedule(dynamic) PRIVATE(TID)
ccc$OMP& copyin(yldot00)
        timebuild=gettime(sec4)
        do ith=1,Nthreads
c wk_(1:neq),igyl_(1:neq,1:2)
c map(from: jcsc(1:neq))
ccc$OMP PARALLEL DO

cDEFAULT(PRIVATE),COPYIN(wk_,igyl_)
      if (DebugOMP.gt.2) then
            write(*,*) 'Calling JacBuilded from thread = ', ith
      endif
      call JacBuilder(ivmin(ith),ivmax(ith),neq, t, yl,yldot00, ml, mu,
     . wk,iJacCol,rJacElem,iJacRow,ith,nnzfinal(ith),nnzmx,nthreads)
c        if (DebugOMP.gt.0) then
c         write(*,'(a,i3,a,i3)')'Task thread=',thread,'with tid=',TID
c        endif
...
c #reduction of jacobian data into single vector
      ...
      nnzcum(1)=nnzfinal(1)-1
      if (DebugOMP.gt.0) then
      write(*,*) 'nnzcum=',nnzcum
      endif
      do ith=2,Nthreads
        nnzcum(ith)=nnzcum(ith-1)+nnzfinal(ith)-1
      enddo
...
c     #rcsc(nnz)= JacElem
      ...
      write(*,*) 'nnzfinal(1)=',nnzfinal(1)
      endif
      rcsc(1:nnzfinal(1)-1)= rJacElem(1:nnzfinal(1)-1,1)
      icsc(1:nnzfinal(1)-1)= iJacCol(1:nnzfinal(1)-1,1)
      if (DebugOMP.gt.0) then
      write(*,*) 'size iJacCol=',size(iJacCol)
      endif
      do ith=2,Nthreads
       rcsc(nnzfinal(ith-1):nnzfinal(ith)-1)=
     .                            rJacElem(1:nnzfinal(ith),ith)
       icsc(nnzfinal(ith-1):nnzfinal(ith)-1)=
     .                            iJacCol(1:nnzfinal(ith),ith)
      enddo
      jcsc(neq+1) = nnzfinal(Nthreads)
      TimeReduce=gettime(sec4)-TimeReduce
      write(*,*)'Time to reduce jac:',TimeReduce


c      # check that we do not build a jacobian too big

c       # check that we have element of the jacobian
c      do jv = 1, nii
c       if (iJacConstructor(iv,jv).gt.0) then
c        rcsc(nnz)= rJacConstructor(iv,jv)
c        icsc(nnz) = jv
c        nnz=nnz+1
c        if (nnz .gt. nnzmx) then
c                   write(STDOUT,*)
c     .             '*** jac_calc -- More storage needed for Jacobian.',
c     .             ' Storage exceeded at (i,j) = (',ii,',',iv,').',
c     .             ' Increase lenpfac.'
c              call xerrab("")
c       endif
c       endif
...
c ----------------------------------------------------------------------
      subroutine JacBuilder(ivmin,ivmax,neq, t, yl,yldot00, ml, mu, wk,
...
c     The Jacobian is stored in compressed sparse row format.
      ...
      real ::yl(*)       # dependent variables
      integer::iJacCol(nnzmx,nthreads),iJacRow(neq,nthreads)
      real:: rJacElem(nnzmx,nthreads)
...
c ... Reset range if extrapolation boundary conditions are used
      ...
      if (DebugOMP.gt.2) then
       write(*,*) 'iii1,ii2 ',ii1,ii2, ' from thread = ', ith
       endif
c ... Initialize all of those right-hand sides to their unperturbed
c     values.
...
c ... Set spatial-location indices for this dependent variable.
         ...
         yc = igyl(iv,2)
      if (DebugOMP.gt.2) then
       write(*,*) 'xc,yc',xc,yc, 'from thread = ', ith
      endif
c ... Save value of dependent variable, then perturb it.
c     The perturbation to the variable is proportional to parameter
c     del and to a measure of the size of the variable.  That measure
...
ccc         call convsr_aux (xc, yc, yl) # test new convsr placement
         if (DebugOMP.gt.2) then
         write(*,*) 'calling pandf1 ', 'from thread = ', ith
         endif
         call pandf1 (xc, yc, iv, neq, t, yl, wk)
ccc         yl(iv) = yold - dyl    # for 2nd order Jac
ccc         call pandf1 (xc, yc, iv, neq, t, yl, yldot0) # for 2nd order Jac
...
c         write(*,*) 'store non-zero elem ', 'from thread = ', TID
         iJacRow(iv,ith) = nnz      # sets index for first Jac. elem. of var. iv
c############### loop ii
         do ii = ii1, ii2
ccc            if (iondenseqn .eq. "inel") then   # skip ion cont. eqn
ccc               ix = igyl(ii,1)               # (but not for guard cells)
