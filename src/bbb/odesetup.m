...
*     exit with a fatal error status.
  ...
  Use(UEint)    # minu,ziin,newgeo,mhdgeo,isallloc,GridFileName
...
* --  local variables
      integer lda, lenk,ngspon,nispon,nuspon,ntgspon,ifld,isor,id,igsp
      character*60 runid
...
c----------------------------------------------------------------------c
     ...
     .                                     nxcore(igrid,2) + 3*nxxpt
         ...
         call gchange("Xpoint_indices",0)
         call readgridpars(GridFileName,runid)  #define/redefine iysptrx1 etc
         nx = nxm - abs(nxomit)
         ny = nym - nyomitmx
...
c ... Check that isupcore=2 is not being used
      do igsp = 1, ngsp
        if (isngcore(igsp) == 2) then
          write(iout,*) "*** isngcore=2 option unvailable; igsp = ",igsp
          call xerrab("")
        endif
      enddo
      ...
      if (inewton(igrid) .eq. 1) then
         if (svrpkg .ne. 'nksol' .and. svrpkg.ne.'petsc') then
            if ((premeth .eq. 'ilut') .or. (premeth .eq. 'inel')) then
              write(iout,*) "*** Invalid option:  premeth=",premeth
              write(iout,*) "When using standard Newton, premeth ",
     .                        "must equal banded."
              call xerrab("")
            endif
         endif
...
cc     do not reset if icntnunk=1 (saved Jac) as storage changes if mmaxu reset
         ...
         write(iout,*) "*** Invalid option:  svrpkg=",svrpkg
         call xerrab("")
      endif
...
cc      write(iout,*) 'Just before nksol,iwork', '  iwork =', iwork
...
cc      write(iout,*) "After gchange; iysptrx1=", iysptrx1
c...  Set indices
      iysptrx1(1) = nycore(igrid)
      iysptrx2(1) = nycore(igrid)
      iysptrx = nycore(igrid)
...
*  -- external routines --
      real ssmin, s2min
*  -- local scalars --
      integer i, iu, ir,ix,iy,igsp
      integer ifld
      integer impflag
      real crni, ctemp, cj, zn_last, proffacx, proffacy, proffacv,tv
      integer ixmp4, jx, jy
...
*  -- check nx, ny, nhsp --
      if (VerboseCall.gt.0) write(iout,*) '**************** calling ueinit ********************'
...
c ------------- New lines for parallel neutral momentum eq.
    ...
    1 continue
      ...
      ifld_lcs = nhsp
      igsp_lcs = nhgsp
cJG memory leak here because mi,zi,znucl of size nisp
      do jz = 1, ngspmx-1    # determine mi, zi, and mg for impurities
         if (nzsp(jz)==0) break
         ifld_fcs = ifld_lcs + 1
         ifld_lcs = ifld_fcs + nzsp(jz) - 1
cJG: add a condition to avoid the memory leak
         if (ifld_lcs.le.nispmx) then
         ...
         write(iout,*) 'no allocation of mi,zi,znucl:ifld>nisp, ifld_lcs:',ifld_lcs,nispmx
         call xerrab('no allocation of mi,zi,znucl:ifld>nisp')
         endif

         if (ngsp .gt. nhgsp) then  # impurity gas is present
            igsp_lcs = igsp_lcs + 1
            if (igsp_lcs.gt.ngspmx) then
            write(iout,*) 'igsp_lcs,ngsp:',igsp_lcs,ngsp
            call xerrab ('uenit:faulty input mg:z')
            endif
...
cJG      if (isimpon .gt. 2 .and. nzspt.gt.0) then #replaced by extending dimension of fnzysi to nzpst+1
             ...
             do iy = 0, ny+1
                do ix = 0, nx+1
                   ...
                   if (nis(ix,iy,ifld) <= 0.) then
                      ...
                      write(iout,*) 'Error at ix=', ix,'  iy=',iy,' ifld=',ifld
                      call xerrab("")
                   endif
                enddo
             enddo
             ...
             do ix = 0, nx+1
                do igsp = 1, ngsp
                   ...
                   if (ngs(ix,iy,igsp) <= 0.) then
                      ...
                      write(iout,*) 'Error at ix=', ix,'  iy=',iy
                      call xerrab("")
                   endif
...
c...  Construct second intermediate velocity grid (xvnrmnx,yvnrmnx)
      do ir = 1, 3*nxpt
cJG wrong index in ixendo (i->ir)
        ...
        do iy = 0, ny+1
          do ix = 0, nx+1
            if (ni(ix,iy,ifld) <= 0) then
              ...
              write(iout,*) 'begins at ix,iy,ifld = ',ix,iy,ifld
              call xerrab("")
            endif
          enddo
        enddo
...
ccc      write(iout,*)  'Just initialized psorc, etc.in ueinit; nisp = ',nisp
...
*     -- routine.  This is necessary for grid sequencing coding.
         if (ismpion.eq.0) then   # MPI parallel version from serial version
           ...
           call allocate
           ifexmain = 0
	   if (icall == 0) write(iout,*) 'UEDGE ',uedge_ver
           icall = 1
         elseif (ismpion.eq.1 .and. icall==0) then
           call init_pll
	   write(iout,*) 'UEDGE version ',uedge_ver
           icall = 1
         endif
...
c_mpi      uedgeComm = COMM
      ...
      mype = MY_PE()
      if (mype .eq. 0) then
!         write(iout, *) "set_uedgeComm: UEDGE has communicator, ", uedgeComm,
...
!      Initialize MPI, Get NPES and MYPE
       if (hascomm==0) then
! c_mpi         write(iout,*) "Initializing MPI on rank ", mype, "."
c_mpi         call MPI_Init(ireturn)
! c_mpi         if(ireturn/=0) write(iout,*) "ireturn=", ireturn
         lcomm = 0
c_mpi         lcomm = mpi_comm_world
         call set_uedgeComm(lcomm)
! c_mpi         write(iout,*) "MPI initialized on rank ", mype, "."
       endif
      return
      end
