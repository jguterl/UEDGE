...
c ... Write uedge plasma data
      if(isechocmdon) then
        cmd="call write(ioutmcnfile("""//trim(bkufile)//""","""//trim(runid)//""")"
        print *,trim(cmd)
      else
	    call write(ioutmcnfile(bkufile,runid)
      end if
...
c ... Set parameters in degas2 background file
      do istra=1,nstra
	    write(ioutnflightstr(istra),'(i0)') mcnflights(istra)
      enddo
...
c     basis subroutine dobalance must be predefined within basis parser
      ...
      if (pnc_step == 1) then	# Write header information into file
        open(unit=pnc_fp,file=trim(pnc_histfile),action="write(iout",
     .        status="replace",iostat=ioerr)
        if(ioerr .ne. 0) then
          write(iout,*) "Trouble opening ",trim(pnc_histfile)," iostat =",ioerr
        endif
        write(ioutpnc_fp,*) "***************************************"
        write(ioutpnc_fp,*) "TIME HISTORY OF RESIDUALS AND SUMMARIES"
        write(ioutpnc_fp,*) "***************************************"
        write(ioutpnc_fp,*) "relax_p  = ", relax_p
        write(ioutpnc_fp,*) "relax_g  = ", relax_g
      endif

      open(unit=pnc_fp,file=trim(pnc_histfile),action="write(iout",status="old",
     .      position="append",iostat=ioerr)
      ...
      write(ioutpnc_fp,*) "***************************************"
      write(ioutpnc_fp,*) 'step = ', pnc_step
      write(ioutpnc_fp,*) 'time = ', pnc_time
      write(ioutpnc_fp,*) "***************************************"
      write(ioutpnc_fp,*) 'fnrm      = ', fnrm_new
      write(ioutpnc_fp,*) 'res_ni    = ', res_ni
      write(ioutpnc_fp,*) 'res_up    = ', res_up
      write(ioutpnc_fp,*) 'res_ti    = ', res_ti
      write(ioutpnc_fp,*) 'res_te    = ', res_te
      write(ioutpnc_fp,*) 'res_phi   = ', res_phi

      if (extneutopt==0) then
        write(ioutpnc_fp,*) 'res_ng    = ', res_ng
        write(ioutpnc_fp,*) 'res_upg   = ', res_upg
      else
        if (get_neutral_sources) then
          write(ioutpnc_fp,*) 'res_sni   = ', res_sni
          write(ioutpnc_fp,*) 'res_smor  = ', res_smor
          write(ioutpnc_fp,*) 'res_smophi= ', res_smophi
          write(ioutpnc_fp,*) 'res_smoz  = ', res_smoz
          write(ioutpnc_fp,*) 'res_sei   = ', res_sei
          write(ioutpnc_fp,*) 'res_see   = ', res_see
        endif
        if (get_neutral_moments) then
          write(ioutpnc_fp,*) 'res_ng   = ', res_ng
          write(ioutpnc_fp,*) 'res_tg   = ', res_tg
          write(ioutpnc_fp,*) 'res_sng   = ', res_sng
          write(ioutpnc_fp,*) 'res_seg   = ', res_seg
        endif
      endif
...
c     write(iout,*) 'PDB open ',trim(filename)
      ...
      write(ioutnxstr,'(i0)') nx+1
      write(ioutnystr,'(i0)') ny+1
      write(ioutnispstr,'(i0)') nisp
      write(ioutngspstr,'(i0)') ngsp
...
c     .   write(iout,*) 'PDB error opening ', filename #call errproc
      ...
      write(ioutnxstr,'(i0)') nx+1
      write(ioutnystr,'(i0)') ny+1
      write(ioutnflstr,'(i0)') nfl
      write(ioutnstrastr,'(i0)') nstra
