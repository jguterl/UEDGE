...
OMPivmin(NchunksJac)   _integer # jacobian rows with ivmin(ithread)<=iv<=ivmax(ithread) are calculated on thread ithread
OMPivmax(NchunksJac)   _integer # jacobian rows with ivmin(ithread)<=iv<=ivmax(ithread) are calculated on thread ithread
OMPLoadWeight(1:NchunksJac)  _real  # weight for load distribution of jacobian calculation among threads
OMPTimeLocalJac(1:NchunksJac)  _real  # runtime for jac calculation on each threads. Used to optimize load distribution of jacobian calculation among threads when AutoBalance=1
